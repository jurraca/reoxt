...
defmodule Reoxt.TransactionFetcher do
  use GenServer
  require Logger
  alias Reoxt.{BitcoinRpcClient, Transactions}

  @doc """
  Starts the transaction fetcher GenServer
  """
  ...
  def handle_cast({:fetch_transaction, txid}, state) do
    Task.start(fn -> do_fetch_transaction(txid) end)
    {:noreply, state}
  end

  @impl true
  def handle_cast({:fetch_block_transactions, block_identifier}, state) do
    Task.start(fn -> do_fetch_block_transactions(block_identifier) end)
    {:noreply, state}
  end

  @impl true
  def handle_cast({:fetch_recent_blocks, count}, state) do
    Task.start(fn -> do_fetch_recent_blocks(count) end)
    {:noreply, state}
  end

  ## Private Functions

  defp do_fetch_transaction(txid) do
    case Transactions.get_transaction_by_txid(txid) do
      nil ->
        case BitcoinRpcClient.get_transaction(txid) do
          {:ok, tx_data} ->
  ...
  defp do_fetch_block_transactions(block_identifier) do
    case BitcoinRpcClient.get_block(block_identifier) do
      {:ok, block_data} ->
  ...
  defp do_fetch_recent_blocks(count) do
    case BitcoinRpcClient.get_blockchain_info() do
      {:ok, info} ->
        ...
        |> Enum.each(fn height ->
          do_fetch_block_transactions(height)
  ...
  defp store_transaction(tx_data, block_height \\ nil) do
    ...
    case Transactions.get_transaction_by_txid(txid) do
      nil ->
        ...
        # Store in database
        case Transactions.create_transaction_with_details(transaction_attrs, inputs_data, outputs_data) do
          {:ok, _transaction} ->
  ...
  defp parse_outputs_data(vout_data) do
    Enum.map(vout_data, fn output ->
      %{
        value: round((output["value"] || 0) * 100_000_000), # Convert BTC to satoshis
        n: output["n"] || 0,
